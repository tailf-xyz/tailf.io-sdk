...
class Log extends EventEmitter {
  ...
  dir(arg, options) {
    ...
    let { id, token, host, verbose_debug } = this.identity();
    if (verbose_debug) {
    ...
    return Promise
            ...
            .tap(() => {
              if (verbose_debug) {
                this.log(`${new Date().toISOString()} - Checking producer...`);
              }

              let p = this.producer.cache.get();
              if (p) {
                if (verbose_debug) {
                  this.log('Producer is still present');
                }
                ...
                return p
                        ...
                        .then((socket) => {
                          if (!_.isNil(socket)) {
                            ...
                            this.log('No need to close socket');
                          }
                        });
              }
            })
