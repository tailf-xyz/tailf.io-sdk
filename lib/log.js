...
class Log extends EventEmitter {
  ...
  pipe(proc, options = {}, cb) {
    ...
    let a = Promise.fromCallback((cb) => stdout.on('end', cb))
      , b = Promise.fromCallback((cb) => stderr.on('end', cb))
      , c = Promise.fromCallback((cb) => proc.on('exit', (code, sig) => cb(undefined, { code, sig })))
      ;
